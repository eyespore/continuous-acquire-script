...
# 菜单UI路径设定
MENU_UI_PATH = 'D:/Desktop/Note/Python/Python_Demos/AcquireScripts/ui/ContinuousAcquireMenu.ui'
...
class AcquireThread(QThread):
    ...
    terminating_signal = pyqtSignal(ThreadID)
    ...
    def __init__(self, **kwargs):
        ...
        self.prog_signal = kwargs['prog_signal']  # 通过进度条信号实时向主线程回传进度信息
        self.log_signal = kwargs['log_signal']  # 通过日志信号向控制台输出信息
        self.status_signal = kwargs['resume_signal']  # 通过恢复信号，在适当的时候恢复主执行按钮
        self.img_signal = kwargs['img_signal']  # 图像信号，用于在获取图像完成后返回图像
        self.error_signal = kwargs['error_signal']  # 错误弹窗信号
        self.schedule_signal = kwargs['schedule_signal']  # 定时任务信号
    ...
    def terminating_thread(self, thread_id: ThreadID):
        """
        通过传入的线程id来终止某个对应的线程
        ...
        self.status_signal.emit(Status.UNAVAILABLE)
        for e_index, element in enumerate(self.threads[thread_id].__closure__):
            # print(e_index, type(element.cell_contents))
            if type(element.cell_contents) is list:
                is_terminated = self.threads[thread_id].__closure__[e_index].cell_contents
    ...
    def single_point_acquire(self, param_json: str):
        ...
        :return: 拍摄获得的图像列表
        """
        # self.error_signal.emit(f'<span style="color: {LogColor.WARNING}; '
        #                        f'font-weight: bold">Unsupported Operation</span>', None)
        # try:
        # 接收数据
        ...
        self.threads[ThreadID.ACQUIRE_THREAD_2] = (
        ...
        # if self.closures[ThreadID.ACQUIRE_THREAD_2].__closure__ is not None:
        #     for item in self.closures[ThreadID.ACQUIRE_THREAD_2].__closure__:
        #         print(item)

        Thread(target=self.threads[ThreadID.ACQUIRE_THREAD_2]).start()
    ...
    def create_single_acquire_strategy(self, **kwargs):
        ...
        pos_top = clamp(int(positions[0]), 0, sy - 1) // y_bin
        pos_left = clamp(int(positions[1]), 0, sx - 1) // x_bin
        pos_bottom = clamp(int(positions[2]), pos_top, sy - 1) // y_bin
        pos_right = clamp(int(positions[3]), pos_left, sx - 1) // x_bin
        ...
        def single_thread_run():
            ...
            if duration != -1:
                ...
                # 终止线程
                self.schedule_signal.emit(duration, lambda: self.terminating_thread(ThreadID.ACQUIRE_THREAD_2))
    ...
    def xy_continuous_acquire(self, param_json: str):
        ...
        try:
            ...
            self.threads[ThreadID.ACQUIRE_THREAD_1] = (
                self.create_xy_acquire_closure(cam_id=cam_id, pieces=pieces, enable_extension=enable_extension,
                                               x_off=x_off, y_off=y_off, extension_unit=extension_unit,
                                               exposure=exposure, x_bin=x_bin, y_bin=y_bin, index=0))

            # 使用策略构建线程并且执行
            Thread(target=self.threads[ThreadID.ACQUIRE_THREAD_1]).start()
    ...
    def create_xy_acquire_closure(self, **kwargs):
        ...
        # 计算步长
        x_step_len = sx // pieces
        y_step_len = sy // pieces

        for line_num in range(pieces):
            # 行循环
            for col_num in range(pieces):
                # 列循环，计算得到目标坐标
                ...
                top = clamp(top, 0, sy - 1)
                left = clamp(left, 0, sx - 1)
                bottom = clamp(bottom, top, sy - 1)
                right = clamp(right, left, sx - 1)
...
class DialogFacade:
    ...
    def warning(self, msg, on_confirm=dummy):
        WarningDialog(self.ui_parent, msg, on_confirm).show()

    def message(self, msg, on_confirm=dummy):
        MessageDialog(self.ui_parent, msg, on_confirm).show()

    def confirm(self, msg, on_ok=dummy, on_cancel=dummy):
        ConfirmDialog(self.ui_parent, msg, on_ok, on_cancel).show()

    def error(self, msg, on_confirm=dummy):
        ErrorDialog(self.ui_parent, msg, on_confirm).show()
...
class MenuUI(QWidget):
    ...
    def __init__(self):
        ...
        super().__init__()
        # GUI属性初始化
        self.ui: QWidget = uic.loadUi(MENU_UI_PATH)
        self.ui.setWindowTitle('Continuous Acquire Menu')
        self.components = self.ui.__dict__  # 将组件作为对象属性
        self.threading_strategy_btn_group = QButtonGroup()
        # 程序输出窗口信息缓存
        self.program_output_cache: List[str] = []

        # exec按钮状态信息
        self.exec_btn_status = Status.VANILLA

        # 构建弹窗门面
        self.dialog_facade = DialogFacade(self.ui)

        # 主线程持有拍摄线程对象引用
        self.acquire_thread = AcquireThread(prog_signal=self.prog_signal,
                                            log_signal=self.log_signal,
                                            resume_signal=self.status_signal,
                                            img_signal=self.img_signal,
    ...
    def connect(self):
        """ 绑定函数与槽 """
        # 拓展命令行窗口
        self.components['enable_extension'].stateChanged.connect(self.check_enable_extension)
        # 将窗口置于最前端
        self.components['stick_on_top'].stateChanged.connect(self.check_stick_on_top)
        # 是否启用多线程策略（现在默认启用多线程）
        # self.threading_strategy_btn_group.buttonClicked.connect(self.check_threading_mode)
        # 通过菜单日志栏打印日志信息
        self.components['program_output'].stateChanged.connect(self.check_program_output)
        # 执行日志栏清空
        self.components['program_output_clean'].clicked.connect(self.click_program_output_clean)
        # 启动程序
        self.components['exec_btn'].clicked.connect(self.click_exec_btn)
        ...
        self.components['splitting_spin_box'].valueChanged.connect(self.val_change_splitting_pieces)

        # 将图像列表回传信号和处理槽函数连接
        self.img_signal.connect(ImageUtil.handle_images)
        # 刷新可用相机下拉选单
        self.components['load_cameras'].clicked.connect(self.load_cameras)
        # 警告弹窗和错误弹窗信号绑定
        ...
        # 是否启用单区域连拍持续时间
        self.components['enable_duration'].stateChanged.connect(self.check_enable_duration)
    ...
    def init_splitting_spin_box(self):
        # 切割规格选择
        splitting_spin_box = self.components['splitting_spin_box']
        splitting_result_label = self.components['splitting_result_label']
        # for fmt in fmts:
        #     splitting_spin_box.addItem(fmt)
        # splitting_spin_box.setCurrentIndex(DEF_SPLITTING_FMT)
        splitting_spin_box.setValue(DEF_SPLITTING_FMT)
        splitting_result_label.setText(f'{DEF_SPLITTING_FMT} x {DEF_SPLITTING_FMT}')
    ...
    def check_enable_duration(self, num: int):
    ...
    # 分片数量改变
    def val_change_splitting_pieces(self, num: int):
        self.components['splitting_result_label'].setText(f'{num} x {num}')

    # 设置进度条
    def set_progress_bar_val(self, val: int):
        self.components['progress_bar'].setValue(clamp(val, 0, 100))

    # 清除控制台输出
    def click_program_output_clean(self):
        # 清空日志窗口内容
        self.program_output_cache.clear()
        self.components['program_output_text'].setText('')

    def click_exec_btn(self):
        if self.get_status() == Status.UNAVAILABLE:
            return

        # 首先查询当前运行模式
        if self.get_status() == Status.TERMINABLE:
            # 停止连续拍摄
            self.status_signal.emit(Status.UNAVAILABLE)
            self.acquire_thread.terminating_signal.emit(ThreadID.ACQUIRE_THREAD_2)

        # 如果状态为VANILLA，那么根据当前界面来决定执行
        if self.get_status() == Status.VANILLA:
    ...
    def xy_continuous_acquire(self):
        ...
        # 分片数量
        pieces = self.components['splitting_spin_box'].value()
    ...
    def check_stick_on_top(self, num: int):
        flag = (num == CHECK_BOX_CHECKED)
        if flag:
            self.ui.setWindowFlags(self.windowFlags() | Qt.WindowStaysOnTopHint)
            self.ui.show()
    ...
    def check_program_output(self, num: int):
        flag = (num == CHECK_BOX_CHECKED)
        if flag:
            self.stick_resize(DEF_WIDTH_WITH_LOG, DEF_HEIGHT)
        else:
    ...
    def check_enable_extension(self, num: int):
        """ XY界面，是否开启边界拓展拍摄 """
        # print(num)
        flag = (num == CHECK_BOX_CHECKED)
        self.components['x_off'].setEnabled(flag)
    ...
    def set_status(self, status: Status):
        ...
        if self.exec_btn_status == Status.UNAVAILABLE:
    ...
    def error(self, msg, on_confirm):
        if on_confirm:
            self.dialog_facade.error(msg, on_confirm)
        else:
            self.dialog_facade.error(msg)

    def warning(self, msg, on_confirm):
        if on_confirm:
            self.dialog_facade.warning(msg, on_confirm)
        else:
            self.dialog_facade.warning(msg)

    # 固定窗口尺寸并且重新渲染
    def stick_resize(self, width: int, height: int):
        self.ui.setMaximumSize(width, height)
        self.ui.setMinimumSize(width, height)
    ...
    # def init_acquire_mode(self):
    #     if acquire_mode == 0:
    #         self.components['enable_multiple_acquire'].setChecked(True)
    #     else:
    #         self.components['enable_single_acquire'].setChecked(True)

    # def init_threading_mode(self):
    #     # 初始化是否采用并行
    #     if threading_mode == 0:
    #         self.components['enable_single_thread'].setChecked(True)
    #     else:
    #         self.components['enable_multiple_thread'].setChecked(True)
    #     self.components['thread_num_spin_box'].setValue(thread_num_spin_box)
    #
    #     self.threading_strategy_btn_group.addButton(self.components['enable_single_thread'])
    #     self.threading_strategy_btn_group.addButton(self.components['enable_multiple_thread'])
    #
    #     if threading_mode == 0:
    #         # self.components['enable_multiple_thread'].setEnabled(False)
    #         self.components['thread_num_spin_box'].setEnabled(False)

    # def check_threading_mode(self, button: QRadioButton):
    #     # print(f'{button.text()} is checked')
    #     if button.text() == 'multiple thread':
    #         self.components['thread_num_spin_box'].setEnabled(True)
    #     else:
    #         self.components['thread_num_spin_box'].setEnabled(False)
